...
const fromJSONString = (json: string): O.Option<t> => {
  ...
  const steps = pipe(
    ...
    getArrayField,
    O.flatMap(
      O.fromPredicate(A.every((x: STEPS_VARIANT) => STEPS_KIND.includes(x))),
    ),
  ) as STEPS_VARIANT[];
...
const askAndMake = async (): Promise<t> => {
  const askSteps = async (
    ...
    else return await askSteps([...prevState, userResponse] as STEPS_VARIANT[]);
  };
...
export const load = async (): Promise<t> => {
  ...
  else return pipe(await askAndMake(), F.tap(flow(toJson, writeToFile)));
};
